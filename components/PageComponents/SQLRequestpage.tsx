...
export default function SQLRequestPage() {
    const defaultPromot = process.env.DEFAULT_SQL_REQUEST;
    ...
    const handleGetAnswer = async () => {
        if (inputValue === '') {
            ...
            try {
                ...
                setInputValue('');
                setIsShowHistory(false);
                const res = await fetch(`/api/openai-sql-request`, {
                ...
                localStorage.setItem('sqlrequest', JSON.stringify([...chatHistory, { Human: inputValue, AI: mainData }]))
                setIsLoading(false);
            } catch {
                handleGetAnswer();
            }
    ...
    const handleClearHistory = () => {
        localStorage.removeItem('sqlrequest')
        setChatHistory([])
        setInputValue('')
        setIsLoading(false)
        setIsShowHistory(false)
    ...
    useEffect(() => {
        const rememberHistory = localStorage.getItem('sqlrequest')
    ...
    return (
        <div className='descSection w-full max-w-7xl flex flex-col justify-evenly gap-6 lg:gap-0 mt-6 lg:mt-0'>
            <div className='flex items-center gap-2'>
                <Icon icon='mdi:sql-query' className='text-purple-500 text-3xl' />
                <div className={`${interB.className} text-[27px] sm:text-3xl text-purple-500`}>SQL Request</div>
            </div>
            <div className='flex flex-col lg:flex-row justify-around gap-4'>
                ...
                <div className='max-w-auto lg:max-w-md text-sm'>
                    ...
                    <div className={`${inter.className} flex flex-col rounded-xl p-3 px-5 mt-1 bg-[#3a0e1f73]`}>
                        <div>Create a SQL request to find all users who live in California and have over 1000 credits:</div>
                    </div>
