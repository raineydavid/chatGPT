...
export default function UltraExtraButton(props: UltraExtraProps) {
    ...
    return (
        <button className={`${inter.className} relative w-max inline-flex items-center justify-center p-4 h-36 min-w-[270px] overflow-hidden text-xl tracking-tighter text-white bg-gray-800 rounded-lg group`} onClick={() => link ? window.open(link, '_blank') : action && router.push(action)}>
            <span className={`absolute w-0 h-0 transition-all duration-500 ease-out bg-purple-500 rounded-full group-hover:w-80 group-hover:h-80`} />
